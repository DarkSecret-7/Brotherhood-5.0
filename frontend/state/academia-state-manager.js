...
/**
 * Academia State Manager
 * Single source of truth for library-related state (bookmarks, browse, preview modal)
 */
class AcademiaStateManager {
    constructor() {
        this.state = {
            // Library
    ...
    saveToLocalStorage() {
        ...
        localStorage.setItem('academia_state', JSON.stringify(dataToSave));
    }

    loadFromLocalStorage() {
        const saved = localStorage.getItem('academia_state');
        if (saved) {
            try {
                const parsed = JSON.parse(saved);
                this.state.bookmarkedGraphMetadata = parsed.bookmarkedGraphMetadata || [];
            } catch (e) {
                console.error('Failed to load academia state from localStorage', e);
            }
        }
    }
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = { AcademiaStateManager };
} else {
    window.AcademiaStateManager = AcademiaStateManager;
}
